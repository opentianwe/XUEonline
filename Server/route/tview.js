...
function URenderTable(usertId, id, emal, yyyy, mm, dd, callback) {
  mysql.queryTimedatabyemal(emal, function (data, err) {
    ...
    if (yyyy != undefined || mm != undefined || dd != undefined) {
      ...
      var dTime = fun_date_to(0);
      for (var i = 0; i < 7; i++) {
        var angData = getNextDate(
          dTime.yyyy + "-" + dTime.mm + "-" + dTime.dd,
          i
        ).split("-");
        var Dates = new Date(angData.join("-"));
        var status = 0;
      ...
      var totime = getNextDate(
        dataTIme[0].yyyy + "-" + dataTIme[0].mm + "-" + dataTIme[0].dd,
        -2
      );
      totime = totime.split("-");
...
//渲染personal.html
async function ProfileRendering(
  res,
  Emal,
  mem,
  oAName,
...
//渲染jppersonal.html
async function JP_ProfileRendering(
  res,
  Emal,
  mem,
  oAName,
...
router.get("/personal.html", function (req, res) {
  if (
    req.signedCookies.malli == undefined ||
    req.signedCookies.malli == "" ||
    req.signedCookies.malli == null
  ) {
    res.redirect("/");
    return;
  }
      ...
      if (data == undefined || data.length == 0 || data == null || err) {
        mysql.queryUserTbyEmal(req.signedCookies.malli, function (data, err) {
          if (data == undefined || data.length == 0 || data == null || err) {
            ...
            mysql.seleteTAppointment(req.signedCookies.malli).then(
              function (datas) {
                var Str = "";
                for (var i = 0; i < datas.length; i++) {
                  ...
                  Str +=
                    ...
                    '" ><td class="timeApp" >' +
                    datas[i].timeApp +
                    '</td><td class="TeacherName">' +
                    datas[i].UserName +
                    '</td><td class="TeacherWeChatID">' +
                    datas[i].UserWeChat +
                    '</td><td class="TeacherSkypeID"><a href="skype:' +
                    datas[i].UserSkypeID +
                    '?add">' +
                    datas[i].UserSkypeID +
                    "</a></td><td>" +
                    ...
                    '</td><td  class="Textval">' +
                    datas[i].Textval +
                    '</td><td  class="time">剩余时间</td><td><button class="layui-btn Historyview">历史查看</button></td><td><button type="button" class="layui-btn privateEvaluation">课程记录</button></td><td> <button type="button" class="layui-btn Studtit">评价</button></td ></tr > ';
                }
                ProfileRendering(
                  res,
                  req.signedCookies.malli,
        ...
        mysql.seleteAppointment(req.signedCookies.malli).then(
          function (datas) {
            var Str = "";
            for (var i = 0; i < datas.length; i++) {
              datas[i].timeApp = crtTimeFtt(datas[i].timeApp);
              Str +=
                ...
                datas[i].Price +
                "</td>" +
                '<td class="button-user"> <button type="button" class="layui-btn Teachertit">评价</button></td><td> <button type="button" class="layui-btn Studate" data-status=0 >取消预约</button> </td><td><button type="button" class="layui-btn newprint" data-status=0 >打印证明</button></td></tr>';
            }
            ProfileRendering(
              res,
              req.signedCookies.malli,
...
router.get("/text.html", function (req, res) {
  ...
  res.render("text.art", {
    data: { Timp: RenderTable(req.query.yyyy, req.query.mm, req.query.dd) },
  });
});
...
router.get("/ter.html", function (req, res) {
  ...
  mysql
    .queryIDbyEmal(Number(req.query.id)) //查询邮箱
    .then(
      function (data) {
...
router.get("/teacherdata.html", function (req, res) {
  ...
  index(req.signedCookies.malli).then((data) => {
    if (!data) {
      res.redirect("/");
      return;
    }
              ...
              function (datas) {
                ...
                for (var i = 0; i < datas.length; i++) {
                  ...
                  Str +=
                    ...
                    '</td><td class="TeacherWeChatID">' +
                    datas[i].UserWeChat +
                    '</td><td class="TeacherSkypeID"><a href="#' +
                    datas[i].UserSkypeID +
                    '">' +
                    datas[i].UserSkypeID +
                    '</a></td><td  class="Rmb">' +
          ...
          function (datas) {
            ...
            for (var i = 0; i < datas.length; i++) {
              ...
              Str +=
                ...
                datas[i].Price +
                "</td> " +
                '<td class="time">剩余时间</td><td class="button-user">  <button type="button" class="layui-btn Teachertit">評価</button></td><td> <button type="button" class="layui-btn Studate" data-status=0 >キャンセル</button></td> <td> <button type="button" class="layui-btn newprint" data-status=0 >受講証明書発行</button></td></tr>';
            }
            JP_ProfileRendering(
              res,
              req.signedCookies.malli,
