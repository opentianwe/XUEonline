...
function getSkyepID() {
    let a = $("#SkyepID").val()
    if (a == "") {
        swal("Sorry!", 'SkypeID必須、持っていない方はダウンロードしてください!', "error")
        return false
...
function getimg() {
    ...
    if (ftype == "jpg" || ftype == "png" || ftype == "jpeg" || ftype == "JPG") {
        ...
        if (size > 3) {
            swal("Sorry!", '画像３Mまでに対応しています', "error")
            return false;
        }
...
$("#avatar").change(function () {
    ...
    $.ajax({
        ...
        success: (d) => {
            if (d.state == 1) {
                swal("Yes!", 'アップロード成功', "success")
            } else {
                swal("Sorry!", 'アップロード失敗', "error")
            }
        }
    })
    ...
    if (getName() && getUserID() && getEngName() && getJapenName() && getNum1() && getNum2() && getSchool() && getHome() && getYearNuber() && getSkyepID() && getYear() && yuyanfun(data) && zheng1() && zheng2() && getSex() && getterData() && getterDatatow()) {
        ...
        $.ajax({
            ...
            success: ((d) => {
                console.log(d)
                if (d.stats == 1) {
                    swal("你好" + $("#Name").val(), '正常に送信されました', "success")

                } else {
                    swal("Error!", 'サーバーエラー', "error")
