...
# 2020/10/14 --Tian
### infop.js 新增取消预约接口/cancel 在498行
# 参数 {
	"Time": "2020-10-13  21:00",
	"id":28
}
# 其中id 为渲染在 列表里的 data-id 右键审查元素能看到
# 需要注意的是 Time 里面的年限和时间的位置是两个空格 渲染上的时候是两个空格但是html页面出来就是一个空格了 请检查  这里错一点就会读取不到数据
# 此接口可靠性待测试 请对接完成后多测试


### 新增 /getime 接口 get方法 用于获取标准的东京时间
# 无参数 返回 {
